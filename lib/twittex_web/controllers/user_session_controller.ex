defmodule TwittexWeb.UserSessionController do
  ...
  def create(conn, %{"user" => user_params}) do
    %{"email_or_password" => email_or_password, "password" => password} = user_params

    if user = Accounts.get_authenticated_user(email_or_password, password) do
      UserAuth.log_in_user(conn, user, user_params)
    else
     # In order to prevent user enumeration attacks, don't disclose whether the email or username is registered.
      render(conn, "new.html", error_message: "Invalid login")
    end
  end
